    ...
    def test_install(self):
        with nested(patch.object(Recipe, "write_buildout_configuration_file_for_production"),
            ...
            with nested(patch.object(BuildoutProject, "_get_platform_arch"),
                        ...
                        patch.object(Project, "_call_rpmbuild"),
                        ) as (get_platform_arch, get_os_string, download, add_directory, call_rpmbuild):
                get_os_string.return_value = 'linux-redhat-6-x64'
                ...
                if not path.exists(self._instance._info.work_dir):
                    makedirs(self._instance._info.work_dir)
                call_rpmbuild.return_value = '\n'.join(["""Processing files: host-power-tools-0.0.1-1""",
           """Checking for unpackaged file(s): /usr/lib/rpm/check-files /tmp/tmp1t4m8ahost-power-tools_rpm_buildroot""",
           """Wrote: /usr/src/redhat/RPMS/i686/host-power-tools-0.0.1-1.i686.rpm"""])
                files = self._instance.install()
                self.assertTrue(files[0].endswith('hello-world-0.0.1-develop-19-gc62dced-linux-redhat-6-x64.rpm'),
                                files[0])
